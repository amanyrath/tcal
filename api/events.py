...
from touchstonecal.cache import CACHE_TTL_SECONDS, get_cached_events, get_last_errors
...
class handler(BaseHTTPRequestHandler):
    def do_GET(self) -> None:  # noqa: N802
        ...
        try:
            ...
            logger.exception("Failed to build events response")
            self.send_error(502, explain=str(exc))
            return

        payload = serialize_events_response(events, updated_at, errors=get_last_errors())
